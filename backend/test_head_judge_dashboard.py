...
jt2_eval = post('/judges/evaluations/mine', jt2, {'team_id': '1', 'competition_id': '3'})
...
reset_eval(hjt, EVAL_ID, 'OPEN', 'Test reset')
r = patch(f'/judges/evaluations/{EVAL_ID}/scores/correct', hjt, {'criterion_id': '1', 'score': '7', 'reason': 'Test correction'})
...
reset_eval(hjt, EVAL_ID, 'OPEN', 'Test reset')
ok = expect_code(f'/judges/evaluations/{EVAL_ID}/scores/correct', hjt, 400, 'PATCH', {'criterion_id': '1', 'score': '6'})
...
ok = expect_code(f'/judges/evaluations/{EVAL_ID}/status', hjt, 400, 'POST', {'new_status': 'OPEN'})
result(f"H10: Reopen without reason rejected: 400", ok)

# H11: Reopen FINALIZED evaluation with reason succeeds
reset_eval(hjt, EVAL_ID, 'FINALIZED', None)
r = post(f'/judges/evaluations/{EVAL_ID}/status', hjt, {'new_status': 'OPEN', 'reason': 'Test reopen'})
...
r = post(f'/judges/evaluations/{EVAL_ID}/status', at, {'new_status': 'OPEN', 'reason': 'Admin reset'})
...
reset_eval(hjt, EVAL_ID, 'LOCKED', None)
reset_eval(hjt, EVAL_ID, 'FINALIZED', None)
ok = expect_code(f'/judges/evaluations/{EVAL_ID}/scores/correct', hjt, 403, 'PATCH', {'criterion_id': '1', 'score': '5', 'reason': 'Try'})
